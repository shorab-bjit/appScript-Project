...
S3HttpPost.prototype.generateSignature = function (string_to_sign) {
  ...
  const date_region_key = CryptoJS.HmacSHA256(this.opts.region, date_key, {
    asBytes: true,
  });
  const date_region_service_key = CryptoJS.HmacSHA256('s3', date_region_key, {
    asBytes: true,
  });
  const signing_key = CryptoJS.HmacSHA256('aws4_request', date_region_service_key, { asBytes: true });
  const signature = CryptoJS.HmacSHA256(string_to_sign, signing_key, {
    asBytes: true,
  });
