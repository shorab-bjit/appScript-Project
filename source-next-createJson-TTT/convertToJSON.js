function convertToJSON() {
  ...
  // Retrieve last values from script properties
  var lastNict = parseInt(PropertiesService.getScriptProperties().getProperty('lastNict')) || 0;
  var lastGoogle = parseInt(PropertiesService.getScriptProperties().getProperty('lastGoogle')) || 0;
  var lastDeepl = parseInt(PropertiesService.getScriptProperties().getProperty('lastDeepl')) || 0;
  var lastMicrosoft = parseInt(PropertiesService.getScriptProperties().getProperty('lastMicrosoft')) || 0;
  ...
  if (startIndex === 0) {
    ...
    // If the process is resumed, set the counters to the last stored values
    nict = lastNict;
    deepl = lastDeepl;
    google = lastGoogle;
    microsoft = lastMicrosoft;
  }
  ...
  try {
    for (let i = startIndex; i < times; i++) {
      ...
      const jsonObject = { sl: rowId, googleId: '', microsoftId: '', deeplId: '', nictId: '', [`${engineType}_engine`]: [] };
      ...
      for (let j = 0; j < headerRow.length; j++) {
        ...
        if (key === `${engineType}_microsoft`) {
          ...
          value === '1' ? (jsonObject.microsoftId = ++microsoft) : (jsonObject.microsoftId = microsoft);
        } else if (key === `${engineType}_google`) {
          jsonObject[`${engineType}_engine`].push(value === '1' ? 'google' : '');
          value === '1' ? (jsonObject.googleId = ++google) : (jsonObject.googleId = google);
        } else if (key === `${engineType}_nict`) {
          jsonObject[`${engineType}_engine`].push(value === '1' ? 'nict' : '');
          value === '1' ? (jsonObject.nictId = ++nict) : (jsonObject.nictId = nict);
        } else if (key === `${engineType}_deepl`) {
          jsonObject[`${engineType}_engine`].push(value === '1' ? 'deepl' : '');
          value === '1' ? (jsonObject.deeplId = ++deepl) : (jsonObject.deeplId = deepl);
        } else {
      ...
      while (retryCount > 0 && !uploadSucceeded) {
        try {
          ...
          if (response == 204) {
            ...
            // Store the current values in script properties for resuming later
            PropertiesService.getScriptProperties().setProperty('lastNict', nict.toString());
            PropertiesService.getScriptProperties().setProperty('lastDeepl', deepl.toString());
            PropertiesService.getScriptProperties().setProperty('lastGoogle', google.toString());
            PropertiesService.getScriptProperties().setProperty('lastMicrosoft', microsoft.toString());
          } else {
